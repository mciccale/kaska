...
static char * leer_string(char *prompt) {
     ...
     // %ms permite leer un string de cualquier tamaño sin reservar memoria
     // pero no funciona en Mac
     // n=sscanf(lin, "%ms", &str);
     //
     // lo sustituimos por una reserva que fija, desgraciadamente, el tamaño
     // máximo del string: pongamos 1023 (+ el nulo)
     str = malloc(1024);
     n=sscanf(lin, "%1023s", str);
     free(lin);
     if (n!=1) return NULL;
     return str;
}
